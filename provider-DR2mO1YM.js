...
var EmojiHandler = class extends Base {
    ...
    */
    async create(name, image) {
        const emoji = await this.client.rest.post(Routes.applicationEmojis(this.client.options.clientId), {
            body: {
                name,
                image
            }
    ...
    constructor(opts) {
        ...
        this.runtime = createChannelNativeApprovalRuntime({
            ...
            buildPendingContent: ({ request }) => {
                ...
                return {
                    body: stripUndefinedFields(serializePayload(buildExecApprovalPayload(isPluginApprovalRequest(request) ? createPluginApprovalRequestContainer({
                        request,
                        cfg: this.opts.cfg,
                        accountId: this.opts.accountId,
                        actionRow
